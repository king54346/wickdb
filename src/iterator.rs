...
/// 键值存储的迭代器trait
pub trait Iterator {
    /// 检查迭代器是否有效，即是否指向数据结构中的一个合法位置。
    fn valid(&self) -> bool;

    /// 将迭代器移动到第一个键。如果数据非空，此方法调用后迭代器应有效。
    fn seek_to_first(&mut self);

    /// 将迭代器移动到最后一个键。如果数据非空，此方法调用后迭代器应有效。
    fn seek_to_last(&mut self);

    /// 将迭代器移动到大于等于指定目标的第一个键。如果存在这样的键，调用后迭代器应有效
    fn seek(&mut self, target: &[u8]);

    /// 分别向前或向后移动迭代器到下一个或前一个键。调用这些方法前必须保证迭代器有效（即 valid() 返回 true）
    /// 传入可变引用，修改迭代器状态来指向下个元素
    fn next(&mut self);


    fn prev(&mut self);

    ///返回当前迭代器位置的键和值。这些方法的调用也要求迭代器当前有效
    fn key(&self) -> &[u8];


    fn value(&self) -> &[u8];

    /// 返回迭代器操作的结果，如果操作成功返回 Ok(())，如果有错误发生返回相应的 Err。
