...
pub struct Options<C: Comparator> {
    ...
    /// 如果为 true，将重用现有的 MANIFEST 和日志文件
    /// 可以显著加快打开速度。
    pub reuse_logs: bool,
...
pub struct ReadOptions {
    ...
    /// 如果“snapshot”为“None”，则从提供的快照开始读取（该快照必须属于正在读取且不得已释放的数据库）。
    /// 如果“snapshot”为“None”，则使用此读取操作开始时状态的隐式快照。
